package org.tensorflow.demo.classescheckpet;

import android.content.Context;
import android.media.AudioManager;
import android.media.MediaPlayer;

import org.tensorflow.demo.R;

/**
 * Created by infoig on 06/04/2018.
 */

public class Texugo {

    private static String nome;
    private static int image;
    private static Context context;
    private static Texugo myInstance;

    //Construtor que recebe parâmetros dos objetos criados na classe Animal
    public Texugo(Context context){
        this.context=context;
    }

    //Permite a criação de objetos na classe Animal
    public static Texugo getInstance(Context context){
        if(myInstance==null){
            myInstance = new Texugo(context);
        }
        return myInstance;
    }
    //Retorna o nome do animal específico para a classe Animal
    public String getNomeTexugo(){
            this.nome="TEXUGO";
        return this.nome;
    }
    //Retorna a imagem do animal específico para a classe Animal
    public int getImagemTexugo( ){

        this.image= R.drawable.texugo;
        return this.image;
    }
    //Retorna o audio do animal específico para a classe Animal
    public void getAudioTexugo(){
        MediaPlayer mPlayer = MediaPlayer.create(context,R.raw.texugo);
        mPlayer.setAudioStreamType(AudioManager.STREAM_MUSIC);
        mPlayer.start();
    }
}
