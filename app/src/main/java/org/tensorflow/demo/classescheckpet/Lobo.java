package org.tensorflow.demo.classescheckpet;

import android.content.Context;
import android.media.AudioManager;
import android.media.MediaPlayer;

import org.tensorflow.demo.R;

/**
 * Created by infoig on 06/04/2018.
 */

public class Lobo {

    private static String nome;
    private static int image;
    private static Context context;
    private static Lobo myInstance;

    //Construtor que recebe parâmetros dos objetos criados na classe Animal
    public Lobo(Context context){
        this.context=context;
    }

    //Permite a criação de objetos na classe Animal
    public static Lobo getInstance(Context context){
        if(myInstance==null){
            myInstance = new Lobo(context);
        }
        return myInstance;
    }
    //Retorna o nome do animal específico para a classe Animal
    public String getNomeLobo(){
        this.nome="LOBO";
        return this.nome;
    }
    //Retorna a imagem do animal específico para a classe Animal
    public int getImagemLobo( ){

        this.image= R.drawable.lobo;
        return this.image;
    }
    //Retorna o audio do animal específico para a classe Animal
    public void getAudioLobo(){
        MediaPlayer mPlayer = MediaPlayer.create(context,R.raw.lobo);
        mPlayer.setAudioStreamType(AudioManager.STREAM_MUSIC);
        mPlayer.start();
    }
}
