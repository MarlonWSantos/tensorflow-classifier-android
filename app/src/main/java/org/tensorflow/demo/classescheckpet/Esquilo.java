package org.tensorflow.demo.classescheckpet;

import android.content.Context;
import android.media.AudioManager;
import android.media.MediaPlayer;

import org.tensorflow.demo.R;

/**
 * Created by infoig on 06/04/2018.
 */

public class Esquilo {

    private static String nome;
    private static int image;
    private static Context context;
    private static Esquilo myInstance;

    //Construtor que recebe parâmetros dos objetos criados na classe Animal
    public Esquilo(Context context){
        this.context=context;
    }

    //Permite a criação de objetos na classe Animal
    public static Esquilo getInstance(Context context){
        if(myInstance==null){
            myInstance = new Esquilo(context);
        }
        return myInstance;
    }
    //Retorna o nome do animal específico para a classe Animal
    public String getNomeEsquilo(){
        this.nome="ESQUILO";
        return this.nome;
    }
    //Retorna a imagem do animal específico para a classe Animal
    public int getImagemEsquilo( ){

        this.image= R.drawable.esquilo;
        return this.image;
    }
    //Retorna o audio do animal específico para a classe Animal
    public void getAudioEsquilo(){
        MediaPlayer mPlayer = MediaPlayer.create(context,R.raw.esquilo);
        mPlayer.setAudioStreamType(AudioManager.STREAM_MUSIC);
        mPlayer.start();
    }
}
