package org.tensorflow.demo.classescheckpet;

import android.content.Context;
import android.media.AudioManager;
import android.media.MediaPlayer;

import org.tensorflow.demo.R;

/**
 * Created by infoig on 06/04/2018.
 */

public class Ovelha {

    private static String nome;
    private static int image;
    private static Context context;
    private static Ovelha myInstance;

    //Construtor que recebe parâmetros dos objetos criados na classe Animal
    public Ovelha(Context context){
        this.context=context;
    }

    //Permite a criação de objetos na classe Animal
    public static Ovelha getInstance(Context context){
        if(myInstance==null){
            myInstance = new Ovelha(context);
        }
        return myInstance;
    }
    //Retorna o nome do animal específico para a classe Animal
    public String getNomeOvelha(){
        this.nome="OVELHA";
        return this.nome;
    }
    //Retorna a imagem do animal específico para a classe Animal
    public int getImagemOvelha( ){

        this.image= R.drawable.ovelha;
        return this.image;
    }
    //Retorna o audio do animal específico para a classe Animal
    public void getAudioOvelha(){
        MediaPlayer mPlayer = MediaPlayer.create(context,R.raw.ovelha);
        mPlayer.setAudioStreamType(AudioManager.STREAM_MUSIC);
        mPlayer.start();
    }
}
