package org.tensorflow.demo.classescheckpet;

import android.content.Context;
import android.media.AudioManager;
import android.media.MediaPlayer;

import org.tensorflow.demo.R;

/**
 * Created by infoig on 06/04/2018.
 */

public class Carneiro {

    private static String nome;
    private static int image;
    private static Context context;
    private static Carneiro myInstance;

    //Construtor que recebe parâmetros dos objetos criados na classe Animal
    public Carneiro(Context context){
        this.context=context;
    }

    //Permite a criação de objetos na classe Animal
    public static Carneiro getInstance(Context context){
        if(myInstance==null){
            myInstance = new Carneiro(context);
        }
        return myInstance;
    }
    //Retorna o nome do animal específico para a classe Animal
    public String getNomeCarneiro(){
        this.nome="CARNEIRO";
        return this.nome;
    }
    //Retorna a imagem do animal específico para a classe Animal
    public int getImagemCarneiro( ){

        this.image= R.drawable.carneiro;
        return this.image;
    }
    //Retorna o audio do animal específico para a classe Animal
    public void getAudioCarneiro(){
        MediaPlayer mPlayer = MediaPlayer.create(context,R.raw.carneiro);
        mPlayer.setAudioStreamType(AudioManager.STREAM_MUSIC);
        mPlayer.start();
    }
}
